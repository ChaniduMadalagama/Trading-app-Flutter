...
class ViewPositionScreen extends StatefulWidget {
  @override
  State<ViewPositionScreen> createState() => _ViewPositionScreenState();
}

class _ViewPositionScreenState extends State<ViewPositionScreen> {
...
class _ViewPositionScreenState extends State<ViewPositionScreen> {
  ...
  void exitPos() async {
    final response = await http.get(Uri.parse('http://prayascapital.com:5000/orders/exit/${widget.id}'));
    if (response.statusCode == 200) {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text('Order Exited Successfully'),
      ));
    } else {
      ScaffoldMessenger.of(context).showSnackBar(SnackBar(
        content: Text(response.body),
